...
import { drawGlyph, glyphRand } from './draw';
import type { AnimationFn, AnimationId } from './types';

/** 共通の退場: フェードしながら少し上へ */
const exitAlpha = (outP: number) => 1 - easeInCubic(outP);

const fadeUp: AnimationFn = ({ ctx, cam, layout, t, inDur, outP, item }) => {
  const stagger = 0.06;
  for (const p of layout.phrases) {
    const e = easeOutCubic(progress(t, p.index * stagger, inDur));
    const rise = 36 * (0.6 + item.energy);
    for (const g of p.glyphs) {
      drawGlyph(ctx, g, cam, { dy: rise * (1 - e) - 16 * easeInCubic(outP), alpha: e * exitAlpha(outP) });
    }
  }
};

const slideMask: AnimationFn = ({ ctx, cam, layout, t, inDur, outP }) => {
  layout.lines.forEach((line, li) => {
    ...
    if (e < 1 || o > 0) {
      if (cam) applyCameraAt(ctx, cam, line.x + line.w / 2, line.y);
      ctx.beginPath();
    ...
    for (const p of line.phrases) for (const g of p.glyphs) drawGlyph(ctx, g, cam, v ? { dy: -50 * (1 - e) } : { dx: -50 * (1 - e) });
    ctx.restore();
  });
};

const phraseStack: AnimationFn = ({ ctx, cam, layout, t, dur, inDur, outP, item }) => {
  ...
  for (const p of layout.phrases) {
    ...
    for (const g of p.glyphs) {
      drawGlyph(ctx, g, cam, layout.vertical ? { dy: d, dx: 12 * easeInCubic(outP), alpha } : { dx: d, dy: -12 * easeInCubic(outP), alpha });
    }
  }
};

const scatter: AnimationFn = ({ ctx, cam, layout, t, inDur, outP, item }) => {
...
const charPop: AnimationFn = ({ ctx, cam, layout, t, inDur, outP, item }) => {
...
const typewriter: AnimationFn = ({ ctx, cam, layout, t, dur, outP }) => {
  const n = layout.glyphs.length;
  const step = Math.min(0.09, (dur * 0.6) / Math.max(n, 1));
  for (const g of layout.glyphs) {
    const e = progress(t, g.index * step, 0.08);
    drawGlyph(ctx, g, cam, { alpha: e * exitAlpha(outP), scale: lerp(1.25, 1, easeOutCubic(e)) });
  }
};

const scaleBurst: AnimationFn = ({ ctx, cam, layout, t, inDur, outP, item }) => {
  ...
  for (const g of layout.glyphs) {
    ...
    drawGlyph(ctx, g, cam, { dx, dy, scale: s, alpha: clamp01(e * 1.5) * (1 - o) });
  }
};

const wave: AnimationFn = ({ ctx, cam, layout, t, inDur, outP, item }) => {
  ...
  for (const g of layout.glyphs) {
    ...
    drawGlyph(ctx, g, cam, { ...xf, alpha: e * exitAlpha(outP) });
  }
};

/** 動きなしの短いフェード（演出なし・可読性重視用） */
const fade: AnimationFn = ({ ctx, cam, layout, t, dur }) => {
  const f = Math.min(0.2, dur * 0.2);
  const alpha = Math.min(progress(t, 0, f), 1 - progress(t, dur - f, f));
  for (const g of layout.glyphs) drawGlyph(ctx, g, cam, { alpha });
};

/** 残響: 文字から残像が広がりながら消え、本体が静かに定着する（エモ系） */
const echo: AnimationFn = ({ ctx, cam, layout, t, inDur, outP, item }) => {
  const o = easeInCubic(outP);
  ...
  for (const g of layout.glyphs) {
    ...
    for (let k = ghosts; k >= 1; k--) {
      ...
      if (aIn > 0.005) drawGlyph(ctx, g, cam, { scale: spreadIn, alpha: aIn });
      if (aOut > 0.005) drawGlyph(ctx, g, cam, { scale: 1 + k * 0.1 * o, alpha: aOut });
    }
    const me = easeOutCubic(progress(t, delay, inDur));
    drawGlyph(ctx, g, cam, { scale: lerp(1.08, 1, me), alpha: me * (1 - o) });
  }
};
