...
> **注意**：初期設定では「書き出し後にアプリ内のデータ（字幕・MV・生成結果）を自動で破棄する」にチェックが入っています。
> 書き出しが終わると、画面がまっさらな状態に戻ります。別の形式でもう一度書き出したい場合は、書き出す前にこのチェックを外してください。
...
- 演出のパターンは保存されません。「演出を再生成」を押すと、前のパターンには戻せません。
- 気に入ったパターンになったら、そのまま書き出してください。
